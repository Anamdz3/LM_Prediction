...
We have a scatter plot for SIMS vs ARM and it seems that the plot is going up to the right.
...
The relationship is a positive relationship, as the is going up to the right.
...
The adjusted R square for SIMS vs ARM is bigger than SIMS vs GRIP.
...
Model1 is nested with Model3 and Model2 is also nested with Model3. We can compare this by using a ANOVA test, as we get our results, we can see that we have low p values. If both models are not nested, then we cannot use the ANOVA test. 
